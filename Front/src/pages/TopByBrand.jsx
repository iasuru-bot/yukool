...
function TopByBrand() {
  const [brand, setBrand] = useState('Nature Valley');
  const [limit, setLimit] = useState(10);
  const [searchBrand, setSearchBrand] = useState('Nature Valley');
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    fetch(`/products/top-by-brand?brand=${encodeURIComponent(searchBrand)}&limit=${searchLimit}`)
      .then(res => {
            ...
            <CardContent sx={{ p: { xs: 1, sm: 2 } }}>
              <Box display="flex" alignItems="center" gap={1} mb={0.5}>
                <Box flex={1} minWidth={0}>
                  ...
                  <Typography variant="body2" color="text.secondary" sx={{ fontSize: { xs: '0.8rem', sm: '0.95rem' }, wordBreak: 'break-word', lineHeight: 1.1 }}>{product.categorie}</Typography>
                </Box>
