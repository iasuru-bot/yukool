...
function TopIngredients({ limit = 10 }) {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    fetch(`/ingredients/top?limit=${limit}`)
      .then(res => {
  ...
  return (
    <Container maxWidth="sm" sx={{ pb: { xs: 8, sm: 2 }, pt: { xs: 2, sm: 4 }, minHeight: '100vh' }}>
      ...
      <Stack spacing={2} sx={{ maxHeight: { xs: 'calc(100vh - 220px)', sm: 'none' }, overflowY: { xs: 'auto', sm: 'visible' } }}>
        {!loading && !error && ingredients.map((ingredient) => (
          <Card key={ingredient.id || ingredient.name} sx={{ borderRadius: 4, background: '#fff', boxShadow: '0 2px 12px 0 rgba(91,91,255,0.07)', px: 1, transition: 'box-shadow 0.2s', '&:active': { boxShadow: '0 4px 16px 0 rgba(91,91,255,0.15)' }, '&:hover': { boxShadow: '0 4px 16px 0 rgba(91,91,255,0.12)' } }}>
            <CardContent sx={{ display: 'flex', alignItems: 'center', p: { xs: 1.2, sm: 2 } }}>
              <Avatar sx={{ bgcolor: '#e3f2fd', color: '#1976d2', mr: 2, width: { xs: 44, sm: 56 }, height: { xs: 44, sm: 56 } }}>
                <SpaIcon />
              </Avatar>
