...
function TopByCategory({ category = 'Bakery', limit = 10 }) {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    fetch(`/products/top-by-category?category=${encodeURIComponent(category)}&limit=${limit}`)
      .then(res => {
  ...
  return (
    <Container maxWidth="sm" sx={{ pb: { xs: 8, sm: 2 }, pt: { xs: 2, sm: 4 }, minHeight: '100vh', px: { xs: 0.5, sm: 2 } }}>
      ...
      <Tabs value={1} variant="fullWidth" sx={{ mb: 2, width: '100%', minWidth: 0, '.MuiTabs-flexContainer': { justifyContent: 'center' } }} TabIndicatorProps={{ style: { height: 3, borderRadius: 2 } }}>
        ...
        <Tab label={<Box sx={{ fontWeight: 700, fontSize: { xs: '0.6rem', sm: '1rem' } }}>Catégorie</Box>} icon={<CategoryIcon />} iconPosition="start" sx={{ minWidth: 0 }} />
        <Tab label={<Box sx={{ fontWeight: 600, fontSize: { xs: '0.6rem', sm: '1rem' } }}>Marque + Catégorie</Box>} icon={<GroupWorkIcon />} iconPosition="start" component="a" href="/top-by-brand-category" sx={{ minWidth: 0 }} />
      </Tabs>
          ...
          <Card key={product.id || product.name} sx={{ borderRadius: 3, background: '#fff', boxShadow: '0 2px 12px 0 rgba(91,91,255,0.07)', px: 0.5, width: '95%', maxWidth: '100%', overflow: 'hidden' }}>
            <CardContent sx={{ display: 'flex', alignItems: 'center', p: { xs: 0.5, sm: 1.5 } }}>
              ...
              <Box flex={1} minWidth={0} sx={{ overflow: 'hidden' }}>
                ...
                <Typography variant="body2" fontWeight={600} color="#5B5BFF" sx={{ fontSize: { xs: '0.9rem', sm: '1.05rem' }, wordBreak: 'break-word', lineHeight: 1.1 }}>{product.brand}</Typography>
                <Typography variant="body2" color="text.secondary" sx={{ fontSize: { xs: '0.75rem', sm: '0.95rem' }, wordBreak: 'break-word', lineHeight: 1.1 }}>{product.category || category}</Typography>
              </Box>
